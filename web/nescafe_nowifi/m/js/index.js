...
(function($){
	...
	function setButton(){
		$('.menu li a').on('click', function(e){
			...
			$('#section2').hide(100);
			window.trackingEvent("m_" + className, "click");
		});
		...
		$('.btn_menu').on('click', function(e){
			...
			if($(this).hasClass('on')){
				...
				$(this).addClass('on');
				$('.location').hide();
				$('.btn_location').removeClass('on');
			};
		});
		
		//setInterval(_txt,4000);
	...
	/*
	function _txt(){
		$('.btn_location').fadeOut(200).fadeIn(200).fadeOut(200).fadeIn(200);
	};
	*/
	...
	function lightboxChange(pEle){
		...
		if(pEle){
			_nowLightBox = pEle;
			Fun.eleFadeIn(_nowLightBox);
			$('.wrap').fadeOut();
		}else{
			delete _nowLightBox;
		}
        if (pEle && pEle.attr('id') == "video" ) {
            $('#video iframe').attr('src', "https://www.youtube.com/embed/3M-xv2PWU2M?rel=0&autoplay=1");
        } else {
            $('#video iframe').attr('src', "");
        }
