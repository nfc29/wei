	...
	switch($_POST['act'])
	{
		...
		case "add":
			...
			if($_POST['pic'] != ''){			
				...
				$magicianObj -> saveImage('../../uploads/media_img/'.$pic, 100);
			...
			if($media_control->media_add($data))
			{
		...
		case "mod":
			...
			if($media_control->media_mod($data))
			{
				echo "mod_ok";	
				exit;
			}
		...
		case "del_img":
			$data['id']=filter_input(INPUT_POST,'id',FILTER_SANITIZE_NUMBER_INT);
			$data['file1']="";
			if(file_exists('../../uploads/media_img/'.$_POST['pic']))
			{
				unlink('../../uploads/media_img/'.$_POST['pic']);
			}
			
			if($media_control->media_delimg_mod($data))
			{
				echo "1";
				exit;
			}
		...
		case "del":
			$data['id']=filter_input(INPUT_POST,'id',FILTER_SANITIZE_NUMBER_INT);
			if($media_control->media_del($data))
			{
				echo "1";
				exit;
				}
