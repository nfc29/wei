...
$result=$login_control->blog_web_list($data);
$data['id']=filter_input(INPUT_GET,'id',FILTER_SANITIZE_NUMBER_INT);
$blog=$login_control->get_blog($data);
$start=$login_control->get_start($data);
if($data['id']==""||$blog=="")
{
?>
<script>
window.location.href="http://msz.com.tw/reported.php?id=<?=$start['b_id']?>";
</script>
<?
}
...
<title>明水然 鍋物。鐵板燒 - 迴響報導 / 部落客推薦</title>
...
<h1><a href="index.php" megais_ga="logo_bt">明水然 鍋物。鐵板燒</a></h1>
  <?include("topMenu.php");?>
    ...
    <ul class="menu2">
        <li><a href="story.php" megais_ga="reported_menu_bt1">品牌故事</a></li>
        <li class="now"><a href="reported.php" megais_ga="reported_menu_bt2">迴響報導</a></li>
        <li><a href="celebrity.php" megais_ga="reported_menu_bt3">名人誌</a></li>
    </ul>
    ...
    <div id="box">
    	<div class="content">
            ...
            <?if($blog['b_url']!=""){?>
			<a href="<?=$blog['b_url']?>" class="link" target="_blank">部落格連結</a>
			<?}?>
            <a href="#" id="close" title="關閉內容">關閉內容</a>
        </div>
        <div class="blogger_bg">
        	<a href="reported.php" class="blogger_bt now">部落客推薦</a>
            <a href="reported_media.php" class="media_bt">媒體報導</a>
            <ul class="bloggers">
				<?for($i=0;$i<count($result);$i++){?>
                <li>
                    <a href="reported.php?id=<?=$result[$i]['b_id']?>"><?=$result[$i]['b_title']?></a>
                    <img src="uploads/blog_img/<?=$result[$i]['b_img']?>" />
                </li>
				<?}?>
            </ul>
