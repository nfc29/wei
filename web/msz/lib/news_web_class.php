...
#################分頁###################
	function news_web_list($data) 
	{
	
		$sqlstr="Select * from `news` where `news_enable`=0 order by `news_cdate` desc;";
	...
	{
		$sqlstr="Select * from `news` where 1=1 and `news_enable`=0 order by `news_id` desc ";
		$stmt=$this->dbh->prepare($sqlstr);
		$stmt->execute();
		return $stmt->fetch(PDO::FETCH_ASSOC);
	}
