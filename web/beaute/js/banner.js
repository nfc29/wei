...
function slideVisual(){
	...
	if(prevNum<0){
		prevNum = $(".banner li").length-1;
	}
	var nextNum = nowSlide+1;
	if(nextNum>=$(".banner li").length){
		nextNum = 0;
	}
	...
	$(".banner li").each(function(){
		...
		if(i==nowSlide){
			...
			//$(this).css({"left":0});
			$(this).fadeOut(500).delay(50).fadeIn(1000,function(){
				if($(".banner li").length>1){
					clearTimeout(slideTimer);
					slideTimer = setTimeout("slideVisualNext()",6000);
				}
			});
		}else{
			$(this).fadeOut(100).fadeOut(100);
		}
		i++;
	});
	...
	$("#visualNavi a").each(function(){
		if(i==nowSlide){
			$(this).html('<img src="images/visual_navi_rollover.png" alt="" width="8" height="8" />');
